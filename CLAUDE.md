...
and `dept_avg/{dept}.js` (that département's historical daily average per
fuel, sets `window.DEPT_AVG_DATA[dept]`). All three chunk types are generated
by `build_site.py` and cached client-side in `Set`s (`loadedStationChunks`,
`loadedHistoryChunks`, `loadedAvgChunks`) once fetched.
...
`selectStation(station)` derives `dept`/`region` from `station.cp` itself
(not from outer search state) and calls `showLocalAvg(dept)` again on
selection — the "Moyennes actuelles" cards and the evolution chart's three
overlay traces (dept: dotted, region: dashed, national: dash-dot) are always
keyed off the station actually clicked, even though in practice this never
diverges from the searched postal code's département (a French postal code's
first two digits are always its département).
